...
export async function POST(request: Request) {
  try {
    ...
    if (!gameId || !playerId || !teamId || !statType) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // 2. Check for existing game stats
      ...
      // 4. Update game score if points were scored
