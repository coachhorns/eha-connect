        ...
        <div className="w-full max-w-[1920px] mx-auto px-6 sm:px-12 lg:px-16 py-12 lg:py-16 relative z-10">
          <div className="flex flex-col md:flex-row gap-8 lg:gap-12 items-end">
            ...
            <div className="relative shrink-0 mx-auto md:mx-0">
              <div className="w-48 h-60 lg:w-56 lg:h-64 bg-surface-raised rounded-sm border-4 border-border-default overflow-hidden relative group shadow-2xl">
                {player.profilePhoto ? (
                  <Image
            ...
            <div className="flex-1 pb-2 text-center md:text-left">
              ...
              <h1 className="text-5xl lg:text-7xl mb-4 tracking-tighter font-bold text-text-primary uppercase">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 lg:gap-8 text-white/80">
                ...
                {currentTeam && (
                  <div className="flex items-center gap-2">
                    ...
                    <Link href={`/teams/${currentTeam.team.slug}`} className="font-bold text-sm uppercase tracking-wider hover:text-text-primary transition-colors">
                ...
                {player.jerseyNumber && (
                  <div className="flex items-center gap-2">
                    <span className="w-8 h-8 flex items-center justify-center bg-surface-overlay rounded-full font-bold text-sm border border-border-default">
                      #{player.jerseyNumber}
                    </span>
                  </div>
                )}
                ...
                <div className="mt-5 flex flex-wrap items-center justify-center md:justify-start gap-3">
                  {player.maxPrepsUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-3 py-1.5 bg-surface-overlay border border-border-default hover:bg-white/20 text-white text-[10px] font-extrabold uppercase tracking-widest rounded-sm transition-colors"
                    >
      ...
      <main className="w-full max-w-[1920px] mx-auto px-6 sm:px-12 lg:px-16 py-12">
        <div className="grid lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <section>
              <div className="flex items-baseline justify-between mb-8">
                <h2 className="text-2xl text-text-primary uppercase tracking-tight font-bold">Season Averages</h2>
              ...
              {careerStats ? (
                ...
                <div className="p-8 border border-dashed border-border-default rounded-sm text-center">
                  <p className="text-text-muted uppercase tracking-widest text-xs font-bold">No verified stats recorded yet</p>
                </div>
              )}
            </section>
                ...
                <div className={!canView ? 'blur-md pointer-events-none select-none relative' : ''}>
                  <table className="w-full text-left">
                    <thead className="bg-surface-glass border-b border-border-subtle">
                      <tr>
                        <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-text-muted">Date</th>
                        <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-text-muted">Opponent</th>
                        <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-text-muted">PTS</th>
                        <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-text-muted">AST</th>
                        <th className="px-6 py-4 text-[10px] font-extrabold uppercase tracking-widest text-text-muted">REB</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {player.gameStats.length > 0 ? player.gameStats.map((stat) => (
                        ...
                        <tr>
                          <td colSpan={5} className="px-6 py-8 text-center text-sm text-text-muted">No recent games recorded</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            {(videos.length > 0 || player.hudlUrl || player.youtubeUrl || player.highlightUrl) && (
              <section>
                <div className="flex items-baseline justify-between mb-8">
                  <h3 className="text-2xl text-text-primary font-bold uppercase tracking-tight">Film Room</h3>
                </div>
                {videos.length > 0 ? (
                  <FilmRoomSection
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <a href={player.highlightUrl || player.youtubeUrl || player.hudlUrl || '#'} target="_blank" rel="noopener noreferrer" className="group relative aspect-video bg-page-bg rounded-sm overflow-hidden border border-border-default cursor-pointer">
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            {/* Athlete Bio/Info */}
            <section className="bg-white border border-border-default rounded-sm p-8 shadow-lg">
            ...
            {player.media.filter(m => m.type === 'PHOTO').length > 0 && (
              <section>
                <h4 className="text-xs font-extrabold uppercase tracking-widest text-text-primary mb-6">Photos</h4>
                <PhotoGallery
                  photos={player.media
